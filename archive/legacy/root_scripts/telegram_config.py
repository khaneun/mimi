...
def is_anthropic_rate_limit_error(error: Exception) -> bool:
    """
    Check if an exception is an Anthropic rate limit error (429).

    Args:
        error: The caught exception

    Returns:
        True if this is an Anthropic rate limit error
    """
    error_str = str(error)
    return "rate_limit" in error_str.lower() or (
        "429" in error_str and ("exceeded" in error_str.lower() or "too many requests" in error_str.lower())
    )


async def send_anthropic_rate_limit_alert(telegram_config: "TelegramConfig", market: str = "KR"):
    """
    Send a Telegram alert when Anthropic API rate limit is hit.
    ...
    try:
        ...
        alert_message = (
            f"🚨 [{market}] Anthropic API 속도 제한 알림\n\n"
            f"Anthropic API 속도 제한에 도달하여 분석 파이프라인이 중단되었습니다.\n\n"
            f"• 오류: rate_limit (HTTP 429)\n"
            f"• 조치 필요: 잠시 후 자동 재시도되거나, Anthropic Console에서 사용량 확인\n"
            f"• https://console.anthropic.com/settings/plans"
        )
        ...
        logger.info(f"[{market}] Anthropic rate limit alert sent to Telegram")
    except Exception as e:
        logger.error(f"[{market}] Failed to send Anthropic rate limit alert: {e}")
