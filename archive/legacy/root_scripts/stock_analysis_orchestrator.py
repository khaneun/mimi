...
# import cores.openai_debug  # noqa: F401 — disabled after migration to Anthropic
import argparse
import asyncio
import json
import logging
...
class StockAnalysisOrchestrator:
        ...
        try:
            ...
            if language == "en":
                try:
                    ...
                    from cores.agents.telegram_translator_agent import translate_telegram_message
                    message = await translate_telegram_message(message, model="claude-haiku-4-20250414")
    ...
    async def generate_reports(self, tickers, mode, timeout: int = None, language: str = "ko", macro_context: dict = None) -> list:
        ...
        Process one stock at a time to prevent API rate limit issues.
        ...
        for idx, ticker_info in enumerate(tickers, 1):
            ...
            reference_date = datetime.now().strftime("%Y%m%d")
            output_file = str(REPORTS_DIR / f"{ticker}_{company_name}_{reference_date}_{mode}_claude-sonnet.md")
...
async def main():
    ...
    parser.add_argument("--no-proxy", action="store_true",
                        help="Disable OAuth proxy (use standard API key)")
