...
# =============================================================================
# MarketPulse Docker Entrypoint Script
# =============================================================================

echo "========================================"
echo "  MarketPulse Container Starting"
echo "  Timezone: $TZ"
echo "  Date: $(date)"
echo "========================================"

# Ensure log directory exists
mkdir -p /app/market-pulse/logs
...
if [ "$ENABLE_CRON" = "true" ]; then
    ...
    if [ -f /app/market-pulse/docker/crontab ]; then
        crontab /app/market-pulse/docker/crontab
        ...
        echo "[WARN] Crontab file not found: /app/market-pulse/docker/crontab"
    fi
...
if grep -q "your_telegram_bot_token" /app/market-pulse/.env 2>/dev/null; then
    echo ""
...
DB_PATH = '/app/market-pulse/stock_tracking_db.sqlite'
...
try:
    ...
    kr_db_schema = load_module_from_path(
        'kr_db_schema',
        '/app/market-pulse/tracking/db_schema.py'
    )
    ...
    us_db_schema = load_module_from_path(
        'us_db_schema',
        '/app/market-pulse/prism-us/tracking/db_schema.py'
    )
