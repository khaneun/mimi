...
pwd
# Example: /home/user/market-pulse
...
# Access container
docker-compose exec market-pulse /bin/bash
```
...
```bash
# Build image
docker build -t market-pulse:latest .

# Run container
docker run -it --name market-pulse-container \
  -v prism-data:/app/market-pulse/data \
  -v prism-db:/app/market-pulse \
  -v $(pwd)/reports:/app/market-pulse/reports \
  -v $(pwd)/pdf_reports:/app/market-pulse/pdf_reports \
  market-pulse:latest

# Access running container (in new terminal)
docker exec -it market-pulse-container /bin/bash
```
...
docker exec market-pulse-container service cron status

# View installed crontab
docker exec market-pulse-container crontab -l

# View cron logs
docker exec market-pulse-container tail -f /var/log/cron.log

# Disable cron (start container without cron)
docker-compose run -e ENABLE_CRON=false market-pulse /bin/bash
```
...
docker exec market-pulse-container crontab /app/market-pulse/docker/crontab
...
Cron job outputs are saved to `/app/market-pulse/logs/`:
...
# Check logs
docker-compose logs -f market-pulse
...
```bash
# Run on host
docker-compose exec market-pulse tar -czf /tmp/backup.tar.gz \
  stock_tracking_db.sqlite reports/ pdf_reports/

docker cp market-pulse-container:/tmp/backup.tar.gz \
...
error mounting "/root/market-pulse/stock_tracking_db.sqlite": not a directory
```
...
docker-compose exec market-pulse ls -la /app/market-pulse/*.sqlite

# Backup DB to host
docker cp market-pulse-container:/app/market-pulse/stock_tracking_db.sqlite ./backup_db.sqlite
```
...
docker-compose exec market-pulse nano /app/market-pulse/.env

# Option 2: Copy to host, edit, then copy back
docker cp market-pulse-container:/app/market-pulse/.env ./.env
# Edit on host
nano .env
# Copy back
docker cp ./.env market-pulse-container:/app/market-pulse/.env
docker-compose restart

# Option 3: Volume mount (after creating files on host)
# Uncomment these lines in docker-compose.yml:
# - ./.env:/app/market-pulse/.env
# - ./mcp_agent.config.yaml:/app/market-pulse/mcp_agent.config.yaml
# - ./mcp_agent.secrets.yaml:/app/market-pulse/mcp_agent.secrets.yaml
```
...
| Access container | 🖥️ Host/Local | `docker-compose exec market-pulse /bin/bash` |
...
# Or
docker build --no-cache -t market-pulse:latest .
```
...
docker-compose exec market-pulse cat /app/market-pulse/.env
...
- **Project**: https://github.com/jacob119/market-pulse
- **Telegram**: https://t.me/stock_ai_agent
- **Issues**: https://github.com/jacob119/market-pulse/issues
...
- **Local environment**: `~/my-path/market-pulse` ✅
- **Docker environment**: `/app/market-pulse` ✅
