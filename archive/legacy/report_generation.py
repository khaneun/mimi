...
# The functions below are no longer called from active pipelines (daily_pipeline, stock_pipeline,
# macro_pipeline, news_crawler, watchlist_analyzer). Retained for reference only.
#
# Previously imported:
...
async def generate_report(agent, section, company_name, company_code, reference_date, logger, language="ko"):
    ...
    # DEPRECATED: mcp-agent LLM attachment removed
...
"""
        ...
        llm = await summary_agent.attach_llm(AnthropicAugmentedLLM)
...
"""
        ...
        llm = await investment_strategy_agent.attach_llm(AnthropicAugmentedLLM)
