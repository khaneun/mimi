...
- 투자 모드 변경(실전↔모의) 시 기존 데이터를 유지한 채 KIS 재조회 → "0원 깜빡임" 현상 제거
- KIS API 호출 중 실전투자 카드 섹션에 반투명 블러 오버레이 + 스피너 표시
- `kisLoading` state 분리로 모드 태그 즉시 반영, 데이터는 sync 완료 후 갱신

---

### 대시보드 카드 패딩 축소

- MetricsCards 카드 내부 패딩 `p-6` → `p-4` 줄임 (남는 여백 최소화)

---

### 하위 메뉴 페이지 헤더 표준화

- 모든 하위 메뉴 진입 시 상단에 **아이콘 + 메뉴명 + 회색 설명 텍스트** 일관 표시
- `PAGE_HEADERS` 테이블로 12개 탭 전체 아이콘/레이블/설명 관리

---

### 포트폴리오 관리 — 모의투자 KIS 현황

- 기존: 실전투자 모드에서만 KIS 계좌 현황 배너 표시
- 개선: 모의투자 모드에서도 현황 표시, **파랑 계열** 색상으로 실전(초록)과 구분
- `portfolioData.kis_mode` 기반 자동 색상 전환; "모의투자" badge 추가

---

### 주식 보유 테이블 간소화

- 기존: 7열 테이블 (수량·매수가·현재가·수익률·비중·전략 등) + 인라인 확장 행
- 개선: **종목명+코드 / 섹터+전략 / 수익률** 3열로 축소
- 행 클릭 시 기존 `StockDetailModal` 팝업으로 매수가·현재가·목표가·보유일·AI 전략 상세 표시
