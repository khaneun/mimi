...
export function PerformanceChart({ data, prismPerformance = [], holdings = [], summary, market = "KR", showIndex = true }: PerformanceChartProps) {
  ...
  // 최신 값 계산
  const latestIndex1 = index1ChartData[index1ChartData.length - 1] ?? { date: "", market_return: 0, prism_return: 0 }
  const latestIndex2 = index2ChartData[index2ChartData.length - 1] ?? { date: "", market_return: 0, prism_return: 0 }
      ...
      <CardContent>
        <ResponsiveContainer width="100%" height={320}>
          <LineChart data={chartData}>
            ...
            <XAxis
              dataKey="date"
              stroke="#e2e8f0" tick={{ fill: '#e2e8f0' }}
              fontSize={11}
              tickLine={false}
              axisLine={false}
              tickFormatter={(value) => {
